...
describe("FileSystemDatasource", () => {
  ...
  test("should save a log in logs-all.log and logs-medium.log and logs-hith.log", () => {
    const logDatasource = new FileSystemDatasource();
    const log = new LogEntity({
      message: "test",
  ...
  test("should return [] if file is empty", async () => {
    const logDatasource = new FileSystemDatasource();
    const logLow = new LogEntity({
      message: "log-low",
      level: LogSeverityLevel.low,
    ...
    const logsEmpty = await logDatasource.getLogs(LogSeverityLevel.medium);
    expect(logsEmpty.length).toBe(0);
  });
  ...
  test("should throw an error if severity level is not defined", async () => {
    ...
    try {
      ...
      expect(errorString).toContain(`Error: Severity level not implemented`);
    }
  });
});
